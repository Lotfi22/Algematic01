function modifierchronique(event,objet) 
{	
	...
	var id_to_hide = "#chronique"+$id;

	var id_nom="#nomchronique"+$id;
    ...
    $.ajax({
        ...
        type:"POST",
        url:"/home/chroniques/modifier/ajax",
...
function supprimerchronique (event,objet) 
{
    ...
    $.ajax({
        ...
        type:"POST",
        url:"/home/chroniques/supprimer/ajax",
        data:{id:$id},
...
function ajouterchronique(event,ojbet) 
{

	event.preventDefault();

	var $nom = $("#nomduchronique").val();

	var $desc = $("#descduchronique").val();
	...
	var to_append = '<tr class="alert alert-success" id="chronique'+new_id+'"><form>{{ csrf_field() }} <td></td><td colspan="2"><div class="form-group col-md-12 col-sm-12">'                                                

	to_append+='<label><textarea type="text" rows="4" name="nom" class=" form-control" id="nomchronique'+new_id+'" value="'+$nom+'">'+$nom+'</textarea></label>'
	...
	to_append+='<td><button class="btn btn-success btn-sm" id="'+new_id+'" onclick="modifierchronique(event,this)"> Enregistrer</button></td>'
	...
	$("#all_the_chroniques").append(to_append).show(1500);
    ...
    $.ajax({
        ...
        type:"POST",
        url:"/home/chroniques/ajouter/ajax",
